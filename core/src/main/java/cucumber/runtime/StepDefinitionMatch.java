...
public class StepDefinitionMatch extends Match implements DefinitionMatch {
    ...
    public void runStep(GherkinDialect i18n, Scenario scenario) throws Throwable {
        try {
            stepDefinition.execute(i18n, transformedArgs(step, i18n, localizedXStreams.get(localeFor(i18n.getLanguage()))));
     ...
     */
    private Object[] transformedArgs(PickleStep step, GherkinDialect i18n, LocalizedXStreams.LocalizedXStream xStream) {
        ...
        if (parameterCount != null && argumentCount != parameterCount) {
            throw arityMismatch(parameterCount, i18n);
        }
    ...
    private CucumberException arityMismatch(int parameterCount, GherkinDialect i18n) {
        ...
        return new CucumberException(String.format(
                ...
                arguments.size(),
                arguments,
                getFirstNonAsterixKeyword(i18n.getStepKeywords()),
                step.getText()
        ));
    }

    private String getFirstNonAsterixKeyword(List<String> stepKeywords) {
    ...
    public StackTraceElement getStepLocation() {
        return new StackTraceElement("✽", step.getText(), featurePath, step.getLocations().get(0).getLine());
    }
